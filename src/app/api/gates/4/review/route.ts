...
const MOCK_DATA = {
  gateNumber: 4,
  phaseName: 'Phase 4 — Detailed Design',
  gateState: 'Locked',
  phaseState: 'Pending',
  inputs: [
    { logicalName: 'DFM, Assembly, Standards, and Supplier-Risk Package', inputRole: 'external', intakeBehavior: 'SI', systemRepresented: 'Standards Library, Supplier Feed', readinessStatus: 'Waiting for Synthetic Sample Ingestion' },
    { logicalName: 'Released Detailed Design Baseline Package', inputRole: 'internal', intakeBehavior: 'UP', systemRepresented: null, readinessStatus: 'Awaiting User Input' },
  ],
  outputs: [
    { outputId: 'mock-out-4-0', outputName: 'Source-Cited DFM and Standards Audit', artifactType: 'XLSX', approvalStatus: 'AwaitingReview' },
    { outputId: 'mock-out-4-1', outputName: 'BOM Health and Manufacturability Report', artifactType: 'DOCX', approvalStatus: 'AwaitingReview' },
  ],
  findings: [],
  openActions: [],
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      gateNumber: GATE, phaseName: 'Phase 4 — Detailed Design',
