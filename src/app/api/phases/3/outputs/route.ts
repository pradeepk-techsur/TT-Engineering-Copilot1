...
export async function GET() {
  const outputs = await db.select().from(phaseOutputs)
    .where(and(eq(phaseOutputs.projectId, 'EVINV-POC-001'), eq(phaseOutputs.phaseId, 3 as any)));
  ...
  if (outputs.length > 2) {
    console.error(`AC-03 VIOLATION: Phase 3 has ${outputs.length} outputs; maximum is 2`);
  }
