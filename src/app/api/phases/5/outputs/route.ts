...
export async function GET() {
  const outputs = await db.select().from(phaseOutputs)
    .where(and(eq(phaseOutputs.projectId, 'EVINV-POC-001'), eq(phaseOutputs.phaseId, 5 as any)));
  ...
  if (outputs.length > 2) {
    console.error(`AC-03 VIOLATION: Phase 5 has ${outputs.length} outputs; maximum is 2`);
  }
