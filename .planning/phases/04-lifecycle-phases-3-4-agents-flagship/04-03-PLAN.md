...
must_haves:
  truths:
    ...
    - "Findings and Actions Workspace (AV-07) shows open blocking actions banner prominently"
    - "A3-001 (blocking=true) appears in blocking actions banner with due gate and evidence requirement"
    - "F4-001 through F4-004 appear in findings table with Seeded badge and Severity badge"
    - "Technical Checklist Workspace (AV-06) shows checklist items for Phase 4 (CDR/PCB Layout) only"
    - "AV-06 shows 'No technical review mapped to this phase' for phases 2, 5, 6, 7, 8, 9"
    - "Playwright test asserts all 4 seeded findings visible after Phase 4 initial run"
    - "Playwright test asserts A3-001 blocking action banner visible"
    - "Playwright test asserts AV-06 only shows checklist for phases 0, 1, 3, 4"
  artifacts:
