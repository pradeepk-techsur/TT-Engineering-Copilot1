...
#
# AGENT-SYNTHESIZED — multi-match: compose + react-next.
# This project declares both docker-compose.yml (postgres + redis + Next.js app)
# and a Next.js package.json. The compose file's app service already runs:
#   npx tsx src/db/migrate.ts && npx tsx src/db/seed.ts && npm run dev
# after db + redis are healthy. docker compose up --build is therefore the
# canonical single exec that handles DB provisioning, migrations, seeding, and
# the Next.js dev server without duplicating any migrate step in this wrapper.
# Using the compose fast path per multi-process-template.md §6.
...
# 0.0.0.0 binding for child containers. The compose file already maps 3000:3000
# (no loopback restriction), so the preview iframe can reach the Next.js app.
# No env preamble needed for this compose stack.
...
# Compose fast path — no heavyweight one-time installs needed.
# docker compose up --build handles image pulls, builds, and all service deps.

# === D-12: idempotent install via lockfile hash + presence check ===
# Compose manages its own image caching via Docker layer cache.
# No npm install step in this wrapper — the compose app service's Dockerfile
# runs `npm ci` during image build. LOCK_FILE and INSTALL_CMD are empty;
# the wrapper proceeds straight to EXEC_CMD.
...
# Compose fast path: docker compose up --build is the single exec command.
# --build is load-bearing: ensures source changes committed during execute/gap-closure
# phases are reflected in the running container, not a stale cached image.
