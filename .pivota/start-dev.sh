...
#
# Agent-fallback synthesis rationale (multi-match: compose + react-next):
#   - This project has both docker-compose.yml (Postgres + Redis + app) and
#     next.config.mjs (Next.js 15). Both catalog entries matched, so agent
#     fallback applies (D-05/D-06).
#   - The compose file is authoritative: it health-gates the DB and Redis,
#     runs `npx tsx src/db/migrate.ts && npx tsx src/db/seed.ts && npm run dev`
#     inside the app service, and exposes port 3000. The wrapper's job is to
#     invoke `docker compose up --build` and let compose own the rest.
#   - `--build` is load-bearing (source changes between boots; stale cached
#     images would loop on old code).
#   - Migrate + seed run INSIDE the compose app service command (AFTER image
#     build / npm ci) — NOT here in the wrapper. This is the correct ordering
#     per references/runtime-environment.md §3.
...
#
# The app service in docker-compose.yml exposes port 3000 via `ports: ["3000:3000"]`
# which binds 0.0.0.0:3000 by default — no loopback-only binding detected.
#
# Next.js HOSTNAME=0.0.0.0 is also exported for the rare case where the app
# service inherits environment from the wrapper's shell context.
...
# No heavyweight one-time installs needed; compose handles image builds and
# npm ci inside the app container. No pre-exec snippet for this entry.

# === D-12: idempotent install via lockfile hash + presence check ===
# For a compose project, docker manages image caching and installs. There is
# no host-side lock file to hash. The install step is a no-op here; the
# EXEC_CMD (`docker compose up --build`) handles all build/install steps
# inside the container on every run (docker's layer cache keeps it cheap).
SENTINEL="/tmp/pivota-setup-sentinel"
LOCK_FILE_PATH=""
INSTALL_PRESENCE_CHECK=""
INSTALL_CMD=''

# No lock file to compare; install step is a no-op for compose projects.
# docker compose up --build handles all dependency resolution inside containers.
